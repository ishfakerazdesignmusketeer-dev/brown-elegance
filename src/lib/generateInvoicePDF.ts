...
export const printInvoicePDF = (order: any) => {
  const doc = new jsPDF();
  const pageW = doc.internal.pageSize.width;

  const orderNumber = order.order_number || order.id?.toString().slice(0,8).toUpperCase() || 'N/A';
  const orderDate = order.created_at
  ...
  const thana = order.thana || order.customer_thana || '';
  const district = order.district || order.customer_district || '';
  const location = [thana, district].filter(Boolean).join(', ');
  const paymentMethod = order.payment_method || order.payment_type || 'COD';
  const paymentStatus = order.payment_status || 'Unpaid';
  const deliveryZone = order.delivery_zone === 'outside_dhaka' ? 'Outside Dhaka' : order.delivery_zone === 'walk_in' ? 'Walk-in' : 'Inside Dhaka';
  const items = order.order_items || order.items || [];
  const subtotal = Number(order.subtotal || order.sub_total || 0);
  const deliveryCharge = Number(order.delivery_charge || order.delivery_fee || 60);
  const discount = Number(order.discount_amount || order.discount || 0);
  const total = Number(order.total_amount || order.total || (subtotal + deliveryCharge - discount));
  const advanceAmount = Number(order.advance_amount || 0);
  const amountToCollect = Number(order.amount_to_collect || (total - advanceAmount) || 0);
  ...
  doc.text(`Date: ${orderDate}`, 20, 58);
  doc.text(`Payment: ${paymentMethod}`, 20, 66);
  if (source) doc.text(`Source: ${source}`, 20, 74);
  doc.text(`Delivery: ${deliveryZone}`, pageW - 20, 50, { align: 'right' });
  doc.text(`Charge: BDT ${deliveryCharge}`, pageW - 20, 58, { align: 'right' });
  ...
  doc.text(`Phone: ${customerPhone}`, 20, 106);
  doc.text(customerAddress, 20, 113);
  if (location) doc.text(location, 20, 120);
  ...
  const itemRows = items.length > 0
    ? items.map((item: any) => {
        ...
        return [name, size, String(qty), `BDT ${price.toLocaleString()}`, `BDT ${(price * qty).toLocaleString()}`];
      })
    : [['No items', '-', '-', '-', '-']];
  ...
  autoTable(doc, {
    ...
    body: itemRows,
    headStyles: { fillColor: [44, 24, 16], textColor: [255, 255, 255], fontStyle: 'bold', fontSize: 9 },
    bodyStyles: { fontSize: 9, textColor: [60, 40, 30] },
    alternateRowStyles: { fillColor: [250, 246, 240] },
    columnStyles: {
      0: { cellWidth: 70 }, 1: { cellWidth: 20, halign: 'center' },
      2: { cellWidth: 15, halign: 'center' }, 3: { cellWidth: 35, halign: 'right' },
      4: { cellWidth: 35, halign: 'right' }
    },
    margin: { left: 20, right: 20 }
  });
  ...
  // Totals box
  doc.setFillColor(250, 246, 240);
  doc.rect(pageW - 90, tableEnd, 70, discount > 0 ? 42 : 35, 'F');
  let ty = tableEnd + 8;
  doc.setFontSize(9);
  doc.setTextColor(80, 60, 50);
  doc.setFont('helvetica', 'normal');
  doc.text('Subtotal:', pageW - 88, ty);
  doc.text(`BDT ${subtotal.toLocaleString()}`, pageW - 22, ty, { align: 'right' });
  ty += 7;
  doc.text('Delivery:', pageW - 88, ty);
  doc.text(`BDT ${deliveryCharge.toLocaleString()}`, pageW - 22, ty, { align: 'right' });
  ty += 7;
  if (discount > 0) {
    doc.text('Discount:', pageW - 88, ty);
    doc.text(`-BDT ${discount.toLocaleString()}`, pageW - 22, ty, { align: 'right' });
    ty += 7;
  }
  doc.setDrawColor(180, 140, 100);
  doc.line(pageW - 88, ty, pageW - 22, ty);
  ...
  doc.setTextColor(44, 24, 16);
  doc.text('TOTAL:', pageW - 88, ty);
  doc.text(`BDT ${total.toLocaleString()}`, pageW - 22, ty, { align: 'right' });
  ...
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(80, 60, 50);
  doc.text(paymentStatus.toUpperCase(), 20, psY + 8);
  if (advanceAmount > 0) {
    doc.text(`Advance Paid: BDT ${advanceAmount.toLocaleString()}`, 20, psY + 16);
    doc.text(`To Collect: BDT ${amountToCollect.toLocaleString()}`, 20, psY + 24);
  }
