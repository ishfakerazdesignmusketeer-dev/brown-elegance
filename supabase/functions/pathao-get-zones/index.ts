...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!city_id) throw new Error("city_id required");

    const supabase = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!);
    const token = await getValidToken(supabase);

    const response = await fetch(`https://api-hermes.pathao.com/aladdin/api/v1/cities/${city_id}/zone-list`, {
      headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
    });

    const result = await response.json();
    if (!response.ok) {
      return new Response(JSON.stringify({ error: "Failed to fetch zones" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    const zones = (result.data?.data || result.data || []).map((z: any) => ({
      zone_id: z.zone_id,
